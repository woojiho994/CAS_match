...
st.caption('第五列为备注,共3种结果：1、不支持该格式文件，请手动查看：说明此类文件不支持解析，请手动查看；2、图片pdf，最好手动复核：说明该pdf为图片，提取正确率较低，视情况可进行人工复核；3、未检测到CAS，请手动检查：说明在该文件中未检测到CAS，请人工确认')

st.caption('提取速度：提取一个电子pdf大约耗时4s，一个扫描版pdf大约耗时10s')
st.divider()
        ...
        #提取docx表格内的内容
            ...
            if cas_extract != []:
                ...
                for item in range(len(cas_extract)):
                    ...
                    #用dataframe承载
                    cas_set = pd.DataFrame({'CAS':{uploaded_file[file].name:cas_iso},'备注':{uploaded_file[file].name:'图片pdf，最好手动复核'}})
                    data = pd.concat([data,cas_set],axis=0)
            else:
                cas_set = pd.DataFrame({'备注':{uploaded_file[file].name:'未检测到CAS，请手动检查'}})
                data = pd.concat([data,cas_set],axis=0)
...
#%%
    for row in data_output.index:
        ...
        for b in target_cas_base.index:
            if data_output.loc[row]['CAS'] == target_cas_base.loc[b]['CAS']:
                data_output.loc[row]['匹配结果'] = '3960种'
                data_output.loc[row]['名称'] = target_cas_base.loc[b]['名称']
        for j in target_cas_pri.index:
            if data_output.loc[row]['CAS'] == target_cas_pri.loc[j]['CAS']:
                data_output.loc[row]['匹配结果'] = '优评优控物质'
                data_output.loc[row]['名称'] = target_cas_pri.loc[j]['名称']
        for p in target_cas_key.index:
            if data_output.loc[row]['CAS'] == target_cas_key.loc[p]['CAS']:
                data_output.loc[row]['匹配结果'] = '重点管控物质'
                data_output.loc[row]['名称'] = target_cas_key.loc[p]['名称']
